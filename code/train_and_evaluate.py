...
print("可見的 GPU 數量:", len(tf.config.list_physical_devices('GPU')))
print("GPU 設備:", tf.config.list_physical_devices('GPU'))
...
def clr(X):
    ...
    return np.log(X + 1e-10) - np.mean(np.log(X + 1e-10), axis=1, keepdims=True)
...
def evaluate_classifier(X_train, y_train, w, X_test, y_test, params):
    ...
    if params['model'] == 'maml':
        ...
        # Apply the same feature filtering to test set
        # Use skp.features which stores the column names after filtering
        X_test_df = X_test_df[skp.features]
    ...
    if params['model'] == 'deepcoda':
        ...
        class SumZeroL1Reg(keras.regularizers.Regularizer):
            def __init__(self, sumzero_lambda=1e0, l1_lambda=1e-2):
                self.sumzero_lambda = tf.cast(sumzero_lambda, K.floatx())
                self.l1_lambda = tf.cast(l1_lambda, K.floatx())

            def __call__(self, w):
                sumzero_reg = 0
                # sumzero_reg += self.sumzero_lambda * K.square(K.sum(w))
                sumzero_reg += self.sumzero_lambda * tf.square(tf.reduce_sum(w))

                l1_reg = 0
                # l1_reg += self.l1_lambda * K.sum(K.abs(w))
                l1_reg += self.l1_lambda * tf.reduce_sum(tf.abs(w))
        ...
        for _ in range(cascade_level):
            # x_log = keras.layers.Lambda(lambda t: K.log(t), output_shape=(X_train.shape[1],))(x)
            # Add epsilon to prevent log(0) which causes NaN
            x_log = keras.layers.Lambda(lambda t: tf.math.log(t + 1e-10), output_shape=(X_train.shape[1],))(x)
    ...
    if params['model'] == 'contrastive':
        ...
        class CustomAugment(object):
            ...
            def _rs(self, x, s=1):
                ...
                x = x / (tf.reduce_sum(x, axis=1, keepdims=True) + 1e-10)
                return x
        ...
        def _am(x, idx1, idx2):
            ...
            n = len(idx1)
            # Fix: add epsilon to both log calls
            x = tf.math.log(x + 1e-10) - tf.reduce_mean(tf.math.log(x + 1e-10), axis=1, keepdims=True)
            x1 = tf.gather(x, idx1)
            x2 = tf.gather(x, idx2)
            p = np.random.rand(n, 1)
            x = p * x1 + (1 - p) * x2
            # Clip values before softmax to prevent overflow
        ...
        def train_step(xis, xjs, model, optimizer, criterion, temperature):
            with tf.GradientTape() as tape:
                ...
                if tf.reduce_any(tf.math.is_nan(zis)) or tf.reduce_any(tf.math.is_nan(zjs)):
                    return tf.constant(float('nan'))

                # normalize projection feature vectors with safe division
                zis_norm = tf.norm(zis, axis=1, keepdims=True) + 1e-8
                zjs_norm = tf.norm(zjs, axis=1, keepdims=True) + 1e-8
                zis = zis / zis_norm
                zjs = zjs / zjs_norm
                ...
                l_pos /= temperature
                # Clip to prevent overflow in softmax later
                l_pos = tf.clip_by_value(l_pos, -50.0, 50.0)
                ...
                for positives in [zis, zjs]:
                    ...
                    l_neg = tf.clip_by_value(l_neg, -50.0, 50.0)
            ...
            if any(g is not None and tf.reduce_any(tf.math.is_nan(g)) for g in gradients):
                return tf.constant(float('nan'))
        ...
        def train_simclr(model, dataset, optimizer, criterion,
            ...
            for epoch in range(epochs):
                ...
                for image_batch in dataset:
                    ...
                    # Check if data_augmentation is a Keras model or custom function
                    if isinstance(data_augmentation, keras.Model):
                        # Keras Sequential model
                        a = data_augmentation(image_batch, training=True)
                        b = data_augmentation(image_batch, training=True)
                    else:
                        # Custom augmentation functions (_am, _cc, _rs, _all)
                        a = data_augmentation(image_batch, idx1, idx2)
                        b = data_augmentation(image_batch, idx1, idx2)

                    loss = train_step(a, b, model, optimizer, criterion, temperature)
                    # Add NaN check and return None to signal failure
                    if tf.math.is_nan(loss):
                        print(f"Warning: NaN loss detected at epoch {epoch}. Returning None.")
                        return None
        ...
        if params.get('init') != 'rand':
            # Retry training up to 3 times if NaN is encountered
            max_retries = 3
            trained_model = None
            ...
            for attempt in range(max_retries):
                ...
                result = train_simclr(model, train_ds, optimizer, criterion,
                                temperature=0.1, epochs=epochs)
                
                if result is not None:
                    epoch_wise_loss, trained_model = result
                    print(f"Training succeeded on attempt {attempt + 1}")
                    break
                else:
                    print(f"Attempt {attempt + 1} failed due to NaN, retrying...")
            
            if trained_model is None:
                raise RuntimeError("Training failed after maximum retries due to NaN losses")
            
            model = trained_model        # classification head
        lreg = LogisticRegressionCV(penalty='l2', Cs=50)
